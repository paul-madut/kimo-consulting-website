...
function Navbar() {
  ...
  return (
    <>
      {/* mobile nav */}
      <div className="relative nav h-fit w-full bg-bgblack text-bgwhite z-20 md:hidden block shadow-sm">
        ...
        {open && (
          <div className="absolute top-auto right-0 w-full bg-white min-h-64 text-bgwhite p-4 z-10">
            <div className="dropdown">
              ...
              <button className="button cursor-pointer bg-teal-800 text-white p-2 rounded-xl py-2 my-2 w-auto h-fit ">
              Call to action{" "}
            </button>
                {Object.entries(navlinks).map(([key, value]) => (
                  <div className="nav__item flex w-full h-fit p-4 border-b border-teal-800 justify-between">
                    <li key={key}>
                      <a href={value}>{key}</a>
                    </li>
      ...
      <div className="relative nav h-fit w-full bg-bgblack text-bgwhite z-20 md:block hidden">
        <div className="nav__container flex justify-between px-4">
          ...
          <div className="right__buttons flex space-x-4">
            ...
            <button className="button cursor-pointer bg-[#e2001a] text-white p-2 rounded-xl py-2 my-2 h-fit ">
              Call to action
            </button>
          </div>
        </div>
